...
namespace Project.DebugUI
{
    ...
    ///   - Global "Drain blood", "Kill", "Reset" actions
    /// All actions go through HealthSystem / SkillSystem public APIs — no
    /// shortcut into private state.
    ...
    public class HealthSkillsDebugPanel : MonoBehaviour
    {
        ...
        [SerializeField] float panelWidth = 520f;
        ...
        void OnGUI()
        {
            ...
            // Lazy-resolve in case the Unit was re-created (re-running scene setup).
        ...
        void DrawGlobalActions()
        {
            ...
            // Modifier readouts: handy to verify formulas while testing.
            GUILayout.Space(6);
        ...
        void KillUnit()
        {
            // Drain blood to zero — exercises the OnBloodDepleted → SetDead path.
            _health.Blood.Drain(_health.Blood.EffectiveMaxBlood + 1f);
        }

        void ResetUnit()
        {
            // Reset skills first so Vitality goes back to L1 (multiplier = 1).
            _skills.ResetAllSkills();

            // Push the L1 multiplier into Health (no preservation: fill to max).
            ...
            // Reset death state. There's no public setter — the cleanest path
            // would be a HealthSystem.Revive() method, but for the MVP debug
            // panel we just re-initialize via the bridge's full path.
            // If still dead, log it: the player should rebuild the scene.
            if (_health.IsDead) Debug.LogWarning("[HealthSkillsDebugPanel] Unit is dead — Reset cannot revive it. Rebuild the scene to restart.");
        }
